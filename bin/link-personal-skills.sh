#!/bin/bash
# Symlink personal skills from <vault>/persona/skills/ into repo's .claude/skills/.
# Repo ships only generic skills (assistant-loop, assistant-test, kb interface stub).
# Personal skills (kb-impl, game-time, ds160, ...) live in the vault next to the
# rest of the persona config (IDENTITY, USER, MEMORY, ...), and this script links
# them back so Claude Code in this repo can see them.
#
...
VAULT_SKILLS="$VAULT_PATH/persona/skills"
