...
Check the preflight results for `BUBBLEWRAP`, `APPLE_CONTAINER` and `DOCKER`, and the PLATFORM from step 1.

- BUBBLEWRAP=installed → use bubblewrap (skip image build entirely — no Docker needed). Continue to 3c.
- PLATFORM=linux + BUBBLEWRAP=not_found → Docker
